...
class SSHSetup:
    ...
    def is_file_exists(self, remote_path):
        stdin, stdout, stderr = self.ssh.exec_command(
            f'sudo test -f {remote_path} && echo "File exists" || echo "File does not exist"'
        )
    ...
    def is_package_installed(self, package_name):
        stdin, stdout, stderr = self.ssh.exec_command(
            f"sudo dpkg -l | grep {package_name}"
        )
        output = stdout.read().decode().strip()
        return output != ""
    ...
    def install_package(self, package_name):
        ...
        if is_installed:
            return True
        stdin, stdout, stderr = self.ssh.exec_command(
            f"sudo apt update -y && sudo apt install {package_name} -y"
        )
    ...
    def remove_package(self, package_name):
        stdin, stdout, stderr = self.ssh.exec_command(
            f"sudo apt remove {package_name} -y && sudo apt autoremove -y && sudo apt purge {package_name} -y"
        )
    ...
    def enable_service(self, service_name):
        stdin, stdout, stderr = self.ssh.exec_command(
            f"sudo systemctl enable {service_name} && sudo systemctl start {service_name}"
        )
        output = stdout.read().decode().strip()
        return output
    ...
    def allow_ipv4_forwarding(self):
        stdin, stdout, stderr = self.ssh.exec_command(
            'echo "net.ipv4.ip_forward = 1" | sudo tee /etc/sysctl.d/99-sysctl.conf && sudo sysctl -p'
        )
    ...
    def copy_to_host(self, **kwargs):
        ...
        sftp.close()
        stdin, stdout, stderr = self.ssh.exec_command(
            f"sudo mv {temp_path} {remote_path}"
        )
        stdout.read().decode().strip()
...
class SetupHost:
    ...
    def setup(self):
        ...
        self.ssh.copy_to_host(
            local_path=self.change_eth_script_path, remote_path="/tmp/change_eth.sh"
        )
...
if __name__ == "__main__":
    ...
    local_path = (
        f"/opt/cloud-iprotate/profile_config/iprotate_{order}_{config_name}/wg0.conf"
    )
