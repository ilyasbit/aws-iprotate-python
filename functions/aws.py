...
class Aws:
    ...
    def format_fingerprint(self, fingerprint):
        ...
        formatted_fingerprint = ':'.join(hex_fingerprint[i:i+2] for i in range(0, len(hex_fingerprint), 2))
    ...
    def set_keypair(self):
        try:
            private_key = paramiko.RSAKey.from_private_key_file(self.config.api_config['sshKeyPath'])
    ...
    def create_security_group(self):
        ...
        for ip_permission in response['SecurityGroups'][0]['IpPermissions']:
            if ip_permission['IpProtocol'] == '-1' and ip_permission['IpRanges'][0]['CidrIp'] == '0.0.0.0/0' and ip_permission['IpRanges'][0]['Description'] == 'Allow all traffic':
    ...
    def run_ansible_playbook(self, playbook_path):
        for i in range(10):
            try:
                ...
                ssh.connect(self.get_instance_address(), username='ubuntu', key_filename=self.config.api_config['sshKeyPath'], timeout=1)
                break
    ...
    def get_instance_address(self):
        response = self.ec2.describe_instances(InstanceIds=[self.aws_config['instanceId']])
    ...
    def associate_ip(self, ip_address):
        ec2 = boto3.client(
            ...
            aws_access_key_id=self.aws_config['accessKey'],
            aws_secret_access_key=self.aws_config['secretKey'])
        response = ec2.associate_address(InstanceId=self.aws_config['instanceId'], PublicIp=ip_address)
        return response
        ...
        for address in response['Addresses']:
            if 'Tags' in address:
                for tag in address['Tags']:
                    if tag['Key'] == 'instance' and tag['Value'] == self.aws_config['instanceId']:
                        try:
            ...
            TagSpecifications=[
                {
                    ...
                    'Tags': [
                        {
                            'Key': 'instance',
                            'Value': self.aws_config['instanceId']
                        }
                    ]
                }
            ]
        )
        self.ec2.associate_address(InstanceId=self.aws_config['instanceId'], AllocationId=response['AllocationId'])
